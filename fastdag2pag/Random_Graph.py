...
class ErdosRenyi(GraphGenerator):
    ...
    def __init__(
        ...
        p_edge: float = None,
        min_num_edges: int = 1,
        def_topological_order: bool = True,
        def_dataframe: bool = False,
        seed: int = None
    ):
        ...
        self.def_dataframe = def_dataframe
        self._manual_seed(seed)

    def get_random_graph(self, def_dataframe: bool=None) -> Union[np.array, pd.DataFrame]:

        if def_dataframe is None:
            def_dataframe = self.def_dataframe
        ...
        if def_dataframe:
            ...
            A = pd.DataFrame(A, index=[f'V{i+1}' for i in range(self.num_nodes)], columns=[f'V{i+1}' for i in range(self.num_nodes)])
    ...
    def set_latent_nodes(self, adj_matrix: pd.DataFrame, latent_rate: float=0.3, num_latent: int=None, selection_bias=False, selection_rate: float=0.1, num_sel: int=None, debug=False) -> pd.DataFrame:
        ...
        # Define Cand_latent as nodes with more than two children
        Cand_latent = set(adj_matrix.columns[adj_matrix.sum(axis=1) >= 2])
        if debug:
        ...
        if selection_bias:
            # If selection bias is True, also include nodes with more than one parent
        ...
        if selection_bias:
            Cand_selection = Cand_selection - set(latent_nodes)
            if len(Cand_selection) >= num_selection:
                # Randomly select selection bias nodes from the candidates
                selection_bias_nodes = random.sample(sorted(Cand_selection), num_selection)
            else:
                # If not enough candidates for selection bias nodes, do not select any
                selection_bias_nodes = list(Cand_selection)
            
        if debug:
            print(f"Selected latent nodes: {latent_nodes}")
            print(f"Selected selection bias nodes: {selection_bias_nodes}")
            
        for node in selection_bias_nodes:
            # Rename the row and column labels from 'Vi' to 'Si'
            new_label = f"S{node[1:]}"  # Replace 'V' with 'S' while keeping the numeric part
...
if __name__ == "__main__":
    ...
    random_graph = er_graph_gen.get_random_graph()
